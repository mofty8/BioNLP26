#!/usr/bin/env bash
# PhenoDP retrieve + Gemma-4-31B pp2prompt_v2 rerank on remaining AllG2P cases.
#
# Input: datasets/RareBench_min/data/ALLG2P_REM.jsonl
#   - 183 cases filtered from allG2P_2026-03-28.csv
#   - Filter: >=3 HPO, PhenoDP-compatible OMIM, limited/moderate/refuted/disputed confidence
#   - Complements ALLG2P.jsonl (definitive/strong, 1771 cases)
#
# PP2Prompt narratives: PP2Prompt/prompts/archive2_merged_en/ALLG2P_REM_XXXX_en-prompt.txt
# Prereq: vLLM API server running on port 8001 (google/gemma-4-31B-it).
...
"${PYTHON_BIN}" run_rarebench_api_rerank.py \
  --run-name "phenodp_gemma4_allg2p_rem_pp2prompt_v2" \
  ...
  2>&1 | tee pr_gemma4_allg2p_rem_pp2prompt_v2.log
