#!/usr/bin/env bash
# Rerank-only run with prompt v3 using Gemma3-27B, negative HPOs INCLUDED.
# Full 6901-case baseline to pair with run_gemma3_v3_no_neg_hpo.sh.
# Reuses retrieval candidates from runs/phenodp_gemma3_prompt_v3_full_rerank.
# Saves to a NEW run dir: runs/phenodp_gemma3_v3_full_rerank_<timestamp>
...
NEW_RUN_DIR="runs/phenodp_gemma3_v3_full_rerank_${TIMESTAMP}"
...
"${PYTHON_BIN}" run_phenodp_benchmark.py \
  ...
  --run-name "phenodp_gemma3_v3_full_rerank" \
  ...
  --prompt-version v3 \
  2>&1 | tee pr_gemma3_v3_full_rerank.log
