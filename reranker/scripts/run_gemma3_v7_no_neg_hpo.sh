#!/usr/bin/env bash
# Rerank-only run with prompt v7 using Gemma3-27B, negative HPOs excluded.
# Reuses retrieval candidates from runs/phenodp_gemma3_v7_full_rerank_20260407_192822.
# Saves to a NEW run dir: runs/phenodp_gemma3_v7_no_neg_hpo_<timestamp>
...
RESUME_RUN_DIR="runs/phenodp_gemma3_v7_full_rerank_20260407_192822"
...
NEW_RUN_DIR="runs/phenodp_gemma3_v7_no_neg_hpo_${TIMESTAMP}"
...
"${PYTHON_BIN}" run_phenodp_benchmark.py \
  ...
  --run-name "phenodp_gemma3_v7_no_neg_hpo" \
  ...
  --prompt-version v7 \
  2>&1 | tee pr_gemma3_v7_no_neg_hpo.log
